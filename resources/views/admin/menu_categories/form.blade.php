<x-default-layout>
    <div class="card mb-5 mb-xl-10">
        <div class="card-header border-0 cursor-pointer">
            <h3 class="fw-bold m-0">{{ isset($menuCategory) ? 'Edit' : 'Create' }} Menu Category</h3>
        </div>
            ...
            <form method="POST" action="{{ isset($menuCategory) ? route('admin.menu-categories.update', $menuCategory->id) : route('admin.menu-categories.store') }}">
                ...
                <div class="card-body border-top p-9">
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="slug" class="form-control form-control-lg form-control-solid"
                                value="{{ old('slug', $menuCategory->slug ?? '') }}" placeholder="Optional">
                        </div>
                    </div>
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <textarea name="description" class="form-control form-control-lg form-control-solid" rows="4">{{ old('description', $menuCategory->description ?? '') }}</textarea>
                        </div>
                    </div>
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    <a href="{{ route('admin.menu-categories.index') }}" class="btn btn-light btn-active-light-primary me-2">Cancel</a>
                    <button type="submit" class="btn btn-primary">{{ isset($menuCategory) ? 'Update' : 'Create' }}</button>
                </div>
            </form>
        </div>
    </div>
